...
first_measurement_visit <- bio_data1 %>%
  group_by(NACCID) %>%
  dplyr::summarize(FirstTauAssayVisit = min(TauAssayNum)) %>%
  ungroup()
```
...
# Open a PDF device
pdf("partial_residuals_plots.pdf", width = 8, height = 6)  # Adjust width and height as needed
...
## use the varables that have the most predictive power
